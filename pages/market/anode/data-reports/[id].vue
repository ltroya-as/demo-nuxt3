...
import { ANODE, MARKET } from '~/constants/services/constants'
...
export default {
  ...
  data() {
    return {
      reportId: null,
      reportCategory: ANODE.categoryReportPricing,
      product: ANODE.slug,
    }
  },
  head() {
    return this.generateSeoInfo({
      title: `${MARKET.name}: ${ANODE.name} Report`,
      description: ANODE.seo.priceAssessments.reportDescription,
    })
  },
  created() {
    this.onInit()
