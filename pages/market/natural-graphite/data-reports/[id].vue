...
export default {
  ...
  data() {
    return {
      reportId: null,
      reportCategory: NATURAL_GRAPHITE.categoryReportPricing,
      product: NATURAL_GRAPHITE.slug,
    }
  },
  head() {
    return this.generateSeoInfo({
      title: `${MARKET.name}: ${NATURAL_GRAPHITE.name} Report`,
      description: NATURAL_GRAPHITE.seo.priceAssessments.reportDescription,
    })
  },
  created() {
    this.onInit()
